...
import modalcross from '../../public/modalcross.svg';

   

  export default function StallModal({ name, stallModal, onClose,onOpen}) {
      ...
      async function fetchdata() {
        try {
          const { data } = await axios.get(
            `http://${API}/api/stalls/modal/${name}`
          );
          setdata(data);
        } catch (error) {
...
overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
 <div className="w-11/12 mb-3 h-fit"><img className="h-6 w-auto float-right" onClick={onClose} src={modalcross} alt="modalcross" /></div>
 <div className="flex flex-row items-center w-11/12 justify-around">
  <div className="w-2/5 h-fit flex flex-col gap-2 items-start justify-around">
    ...
    <div className="w-7/12 mb-2 flex flex-row items-start justify-between h-fit">
       <a href={data.twitter} target="_blank"><img className="h-8 w-auto" src={twitter} alt="twitter"/></a>  
     <a href={data.discord} target="_blank"><img className="h-8 w-auto" src={discord} alt="discord"/></a> 
    <a href={data.github} target="_blank"><img className="h-8 w-auto" src= {github} alt="github"/></a>  </div>
